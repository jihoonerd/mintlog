from markdownx.admin import MarkdownxModelAdmin
from .models import Portfolio
import tagulous


class PortfolioAdmin(MarkdownxModelAdmin):
    list_display = ('title', 'author', 'portfolio_tag', 'published_at', 'status')
    prepopulated_fields = {'slug': ('title',)}
    ordering = ['-published_at']
    list_filter = ['portfolio_tag', 'published_at', 'status']
    search_fields = ['title', 'description', 'contents']


# Use tagulous.admin to employ the functionality provided by tagulous package.
tagulous.admin.register(Portfolio, PortfolioAdmin)